...
| R-22 | vmbrMGMT + nic4 (enx0060dd44ecce) still active on srv-proxmox-poc-01 — live link to Arista prod fabric VLAN 600. Risk of PoC traffic injection into prod broadcast control plane. | High | Critical | 🔴 Critical | **Decision 2026-04-01:** Rename `vmbrMGMT` → `vmbrFAB` (supervision bridge name in VLAN plan). Disable bridge + bring nic4 (enx0060dd44ecce) down. Re-enable only when PoC fabric is physically wired. Manual step on srv-proxmox-poc-01 (SSH/console). | Infra | Open |
